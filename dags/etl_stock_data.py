...
# Transform task: Combine the stock data of AAPL and IBM into a single list
@task
def transform(aapl_data, ibm_data):
    return aapl_data + ibm_data  # Concatenating the two lists
...
def load(records):
    ...
    conn = hook.get_conn()
    cur = conn.cursor()
    target_table = "lab2.raw_data.market_data"
